...
while wb_robot_step(TIME_STEP) ~= -1
  ...
  elseif(left > BLOCK_THRESH && right > BLOCK_THRESH && front > BLOCK_THRESH)
       %Turn around (right)
  elseif(right - left > BLOCK_THRESH && front > BLOCK_THRESH)
      %Turn left
  elseif(left-right > BLOCK_THRESH && front > BLOCK_THRESH)
      %Turn right
  elseif(front > BLOCK_THRESH)
      %Turn tight (this is default case, where going fw is blocked, but we
